...
- 支持自定义 GPU 列表和每卡并行度
- 结果与日志写到新的 run 目录
- run 目录内自动创建 data 软链接（指向原项目 data）
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run model_b(100) finetune for day8-12 and seed0-9 in parallel.")
    ...
    parser.add_argument("--run_tag", type=str, default=None, help="Default: modelb100_repro_<timestamp>")
    parser.add_argument("--scripts_dir", type=str, default=None, help="Default: <orig_base>/scripts")
    parser.add_argument("--model_b_root", type=str, default=None, help="Default: <orig_base>/scripts/outputs/model_b")
    ...
    parser.add_argument(
        ...
        default=[],
        help="Extra args passed to main_pipeline.py. Use: --extra_args -- --arg val",
    )
...
def is_model_b_loadable(model_b_path: Path) -> bool:
    # LoRA: adapter_config.json + head.pt
    lora_ok = (model_b_path / "adapter_config.json").exists() and (model_b_path / "head.pt").exists()
    # non-LoRA: best_model.pt
    full_ok = (model_b_path / "best_model.pt").exists()
    return lora_ok or full_ok
...
def build_tasks(
    ...
    for seed in seeds:
        for day in days:
            ...
            model_b_path = model_b_root / f"slpe-5days-seed{seed}"
            log_file = logs_dir / f"modelb100_seed{seed}_day{day}.log"
...
def run_one_task(
    ...
    *,
    ...
    try:
        ...
        if not task.model_b_path.exists():
            return False, f"missing model_b_path: {task.model_b_path}"
        if not is_model_b_loadable(task.model_b_path):
            return False, f"model_b not loadable (need head.pt or best_model.pt): {task.model_b_path}"
...
def write_tasks_csv(tasks: List[Task], csv_path: Path) -> None:
    ...
    with open(csv_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["seed", "day", "gpu", "model_a_path", "model_b_path", "log_file"])
        for t in tasks:
            writer.writerow([t.seed, t.day, t.gpu, str(t.model_a_path), str(t.model_b_path), str(t.log_file)])
...
def main() -> None:
    ...
    model_b_root = Path(args.model_b_root).resolve() if args.model_b_root else (orig_base / "scripts" / "outputs" / "model_b")
    run_root = Path(args.run_root).resolve() if args.run_root else (orig_base / "outputs" / "repro_runs")
    run_tag = args.run_tag or f"modelb100_repro_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"model_b_root={model_b_root}")
